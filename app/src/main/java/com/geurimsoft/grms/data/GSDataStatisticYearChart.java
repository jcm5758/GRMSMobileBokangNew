...
public class GSDataStatisticYearChart {
	...
	public GSDataStatisticYearChart(Context context, String _mDate, StatsListData _statsListData) {
	...
	private void makeChart(StatsListData _statsListData) {
		...
		multiRenderer.setYAxisMax(Maxnode+1500);
		multiRenderer.setXAxisMin(-0.5);
		multiRenderer.setXAxisMax(itemCount);
		multiRenderer.setBarSpacing(0.5);
		multiRenderer.setBackgroundColor(Color.TRANSPARENT);
		multiRenderer.setMarginsColor(this.mContext.getResources().getColor(
				R.color.transparent_background));
		multiRenderer.setApplyBackgroundColor(true);
		multiRenderer.setMargins(new int[] { 30, 30, 30, 30 });

		for (int i = 0; i < itemCount; i++) {
			multiRenderer.addXTextLabel(i, (i + 1) + "월");
		}
